#include "PlayColorMatch.h"
#include <iomanip>


PlayColorMatch::PlayColorMatch(controller* pCont) : operation(pCont)
{
	Correct = 0;
	Wrong = 0;
	Score = 0;
...
void PlayColorMatch::Execute()
{
	...
	{
		pUI->PrintMessage("Select all " + PlayingColor.Color2String() + " Figures");
		...
		while (Count)
		{
			...
			{
				pUI->PrintMessage("Correct Ans = " + to_string(Correct) + "        "
					+ "Wrong Ans = " + to_string(Wrong) + " Score = " + to_string((3 * Correct) - Wrong) + "        It's Play Mode : Please select any icon from the tool bar");
				pUI->ClearDrawArea();
...
void PlayColorMatch::Undo()
{
}

void PlayColorMatch::Redo()
{
}
